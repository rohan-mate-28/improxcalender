...
export async function readJson<T>(key:string,fallback:T):Promise<T>{if(isNetlify){const v=await getStore("improx-calendar").get(key,{type:"json"});return (v as T|null)??fallback}return localRead(key==="events"?"events.json":"calendars.json",fallback)}
export async function writeJson<T>(key:string,value:T){if(isNetlify){await getStore("improx-calendar").setJSON(key,value);return}await localWrite(key==="events"?"events.json":"calendars.json",value)}